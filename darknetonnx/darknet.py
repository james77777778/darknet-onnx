...
def export_to_onnx(
    cfgfile, weightfile, outputfile, batch_size=1, to_float16=False
):
    # load model
    ...
    if version.parse(torch.__version__) < version.parse("1.9.1"):
        raise NotImplementedError(
            f"torch version must be >= 1.9.1 but got: {torch.__version__}"
        )
    ...
    elif version.parse(torch.__version__) >= version.parse("1.12.0"):
        torch.onnx.export(
            loaded_model,
            x,
            outputfile,
...
def yolo_forward_dynamic(
    output, num_classes, anchors, num_anchors, scale_x_y, version="yolov4"
):
    bxy_list = []
    bwh_list = []
    det_confs_list = []
...
class Darknet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            return torch.cat(
                final_outputs, dim=1
            )  # concate all yolo layer outputs
        ...
        for block in blocks:
            if block["type"] == "net":
                ...
                if activation == "leaky":
                    ...
                    model.add_module(
                        "mish{0}".format(conv_id), nn.Mish(inplace=True)
                    )
                elif activation == "logistic":
                    model.add_module(
                        "logistic{0}".format(conv_id), nn.Sigmoid()
                    )
                elif activation == "linear":
                    model.add_module("linear{0}".format(conv_id), nn.Identity())
                else:
                    print(
                        "convolution has no activation: {}".format(activation)
                    )
                ...
                if pool_size == 2 and stride == 1:
                    model.add_module(
                        "zeropad{0}".format(conv_id), nn.ZeroPad2d((0, 1, 0, 1))
                    )
                model.add_module(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "ONNX model will be dynamic. Skip 0"
        ),
    )
    parser.add_argument(
